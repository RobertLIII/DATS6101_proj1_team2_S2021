...
y.pred <- predict(mod, data.test)

y.test <- data.test$Price

# drop Bedroom2
mod2 <- lm(Price ~ Rooms + Car 
          + Distance   + Car + BuildingArea 
          + Lattitude + Longtitude + Propertycount 
          + factor(Regionname) , data=data.train["13246",])

summary(mod2)
vif(mod2)

#  mod 2 with rooms*region interaction -  failed model, drop interaction
mod3 <- lm(Price ~ Rooms*factor(Regionname) + Car 
          + Distance   + Car + BuildingArea 
          + Lattitude + Longtitude + Propertycount, data=data.train)
