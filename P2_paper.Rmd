...
In project one, we do this by determining how different factors may affect home sales price by building linear models. This time, we attempt to improve on those linear models and create some classification models. 
...
- Improve on our results from project one. 
...
```
The Bayesian Information Criterion, or BIC for short, is a method for selecting a model. Lower BIC is best for model, so I selected the top five variables among all variables in the graph. The best model includes five variables: `Distance`, `Rooms`, `Lattitude`, `BuildingArea`, `YearBuilt`. 
...
Regularization is to reduce the complexity of the model by adjusting the model parameters (number and size) to achieve the effect of avoiding overfitting. The most basic regularization method is to add a penalty term to the original target (cost) function to punish the model with high complexity. The regression problem is a process of function fitting. We hope that the model should not be too complicated, otherwise it is prone to overfitting. So we need to add a regularization term, and different regularization terms produce different regression methods. Lasso adds an L1 regularization term. L1 regularization can generate a sparse weight matrix, that is, a sparse model can be used for feature selection. To a certain extent, L1 can also prevent overfitting (Tibshirani, R. 1996).
...
Using cross-validation to select lambda, indicates that the unregularized model does a good job and that within one standard error at nine variables is also a good choice. The plot shows how many non-zero variables are in the model at the top. When the log lambda value belongs to -3 and -2, the no-zero variable is between 9 and 8. Indeed the values of the coefficients for the best model of nine variables can be extracted.
## Coefficients
...
This is the predicted distribution of different price ranges after prediction. Among them, it is mainly concentrated in 600,000 to 1,200,000.
## Lasso Prediction
...
An issue is that the function of normalized are influencing the accuracy of prediction price. It can be seen from the non-normalized prediction results that the error between the predicted value and the actual value is relatively large, and the corresponding fitting curve between the overall predicted data and the true value is a non-proportional function curve.
...
Combining the distribution of the predicted value with the real one, it can be known that the normalized prediction accuracy is higher than the non-normalized prediction.
...
xkabledply(mod.RF.cat$confusion)
